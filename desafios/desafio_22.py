...
def encontrar_modelo(caracteristica):
    for key, value in caracteristicas_modelos.items():
        result = re.compile(re.escape(key), re.IGNORECASE)
        if result.search(caracteristica):
            return value
    return 'Nenhum valor foi encontrado de acordo com as caracteristicas fornecidas.'
